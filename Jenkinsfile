...
node {
    ...
    stage("Build dockers") {
        tryStep "build", {
            def api = docker.build("build.datapunt.amsterdam.nl:5000/datapunt/signals-maintenance-page:${env.BUILD_NUMBER}", "api")
            api.push()
            api.push("acceptance")
        }
    }
...
if (BRANCH == "master") {
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                def image = docker.image("build.datapunt.amsterdam.nl:5000/datapunt/signals-maintenance-page:${env.BUILD_NUMBER}")
                image.pull()
    ...
    node {
        stage('Push production image') {
            tryStep "image tagging", {
                def api = docker.image("build.datapunt.amsterdam.nl:5000/datapunt/signals-maintenance-page:${env.BUILD_NUMBER}")
                api.pull()
                api.push("production")
                api.push("latest")
            }
